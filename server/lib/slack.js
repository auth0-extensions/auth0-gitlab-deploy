...
const createPayload = (progress, extensionUrl) => {
  ...
  if (progress.error) {
    msg.attachments.push({
      color: '#F35A00',
      fallback: `GitLab to Auth0 Deployment failed: ${progress.error.message}`,
      text: `GitLab to Auth0 Deployment failed ${details}`,
      fields: [
    ...
    const fields = [
      ...
      { title: 'Branch', value: progress.branch, short: true },
      { title: 'ID', value: progress.id, short: true },
      { title: 'Commit', value: `<https://gitlab.com/${progress.repository}/commit/${progress.sha}|${progress.sha}>`, short: true }
    ];
    ...
    msg.attachments.push({
      ...
      text: `GitLab to Auth0 Deployment ${details}`,
      fields
    });
  }
