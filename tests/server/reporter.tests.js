...
const checkData = (result, isError, done) => {
  ...
  if (slack) {
    ...
    if (isError) {
      expect(slack.attachments[0].fallback).toEqual('Gitlab to Auth0 Deployment failed: Testing error');
      expect(slack.attachments[0].text).toEqual('Gitlab to Auth0 Deployment failed: (<https://wt.example.com/login|Details>)');
      ...
      expect(slack.attachments[0].fallback).toEqual('Gitlab to Auth0 Deployment');
      expect(slack.attachments[0].text).toEqual('Gitlab to Auth0 Deployment (<https://wt.example.com/login|Details>)');
